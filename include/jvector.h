...
#define jvector_push_back(vec_) ({ \
    ...
    assert((vec_)->items_); \
    __typeof__((vec_)->size_[0]) *ret_ = &(vec_)->items_[(vec_)->count_]; \
    (vec_)->count_++; \
    ret_; \
})
...
#define jvector_get_val(vec_, i_) ((__typeof__((vec_)->size_[0]) *)(vec_)->items_)[i]

#define jvector_ensure(vec_, n_) ({ \
    if ((vec_)->count_ + n_ > (vec_)->capacity_) { \
        (vec_)->capacity_ = (vec_)->capacity_ + n_; \
        (vec_)->items_ = (__typeof__((vec_)->size_[0]) *)realloc((vec_)->items_, (vec_)->capacity_ * sizeof((vec_)->size_[0])); \
    } \
})
